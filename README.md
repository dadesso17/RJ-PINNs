...
RJ-PINNs, introduced by Dadoyi Dadesso, is a pioneering framework using Jacobian-based least-squares (TRF) to directly minimize residuals without traditional loss functions. This method is the This method eliminate gradient optimizers in PINNs or AI, offering unmatched robustness for inverse PDE problems.
...
- **No Gradient Optimizers**: Eliminate the need for gradient optimizers in PINNs or AI.
...
`r(θ) = [r_data(θ), r_physics(θ), r_bc(θ), r_ic(θ)]ᵀ`

**Objective:** Solve `r(θ) ≈ 0` via least-squares minimization (i.e., `min_θ ||r(θ)||²`)
...
> [!IMPORTANT]
> 🚀 **RJ-AI = Generalization of RJ-PINNs**
