...
class RJ_PINNs:
    ...
    def R_physic(self, x, t):
        ...
        f = lambda_1*u_t + u* u_x - lambda_2 * u_xx
    ...
    def get_weights(self):
        theta = [*self.weights, *self.biases,self.lambda_1, self.lambda_2]
        return tf.concat([tf.reshape(var, [-1]) for var in theta], axis=0)

    def set_weights(self, flat_weights):
        idx = 0
        for var in [*self.weights, *self.biases, self.lambda_1,self.lambda_2]:
            shape = tf.shape(var)
    ...
    def R(self, p, print_loss=True):
     ...
     u_icd=tf.convert_to_tensor(np.sin(np.pi* self.x_ic)+0.5*np.sin(4*np.pi*self.x_ic), dtype=tf.float32)
    ...
    def J(self, p):
        ...
        jac_list = tape.jacobian(r, [*self.weights, *self.biases,self.lambda_1 ,self.lambda_2])
