...
class RJ_PINNs:
    ...
    def R(self, p, print_loss=True):
     ...
     u_icp=self.R_bc(self.x_ic, self.t_ic)
     u_bc0_xxd=tf.convert_to_tensor(0, dtype=tf.float32)
     u_bc1_xxd=tf.convert_to_tensor(0, dtype=tf.float32)
     ...
     with tf.GradientTape(persistent=True) as tape:
          tape.watch(self.t_bc0)
          tape.watch(self.x_bc0)


          u_bc0p=self.R_bc(self.x_bc0, self.t_bc0)

          u_bc0_xp=tape.gradient(u_bc0p,self.x_bc0)
     #print(u_ic_tp.shape)
     u_bc0_xxp=tape.gradient(u_bc0_xp,self.x_bc0)


     del tape
     with tf.GradientTape(persistent=True) as tape:
          tape.watch(self.t_bc1)
          tape.watch(self.x_bc1)


          u_bc1p=self.R_bc(self.x_bc1, self.t_bc1)

          u_bc1_xp=tape.gradient(u_bc1p,self.x_bc1)
     #print(u_ic_tp.shape)
     u_bc1_xxp=tape.gradient(u_bc1_xp,self.x_bc1)
    ...
    # Combine residuals
     r = tf.concat([r_data, r_physic,r_bc0, r_bc0_xx,r_bc1,r_bc1_xx,r_ic,r_ic_t], axis=0)
     #print(r.shape)
     ...
     if print_loss and self.iteration % 10 == 0:
        ...
        print(f"r_ic:{np.mean(r_ic.numpy())}")
        print(f"r_ic_t:{np.mean(r_ic_t.numpy())}")
        print(f"r_bc0:{np.mean(r_bc0.numpy())}")
        print(f"r_bc0_xx:{np.mean(r_bc0_xx.numpy())}")
        print(f"r_bc1:{np.mean(r_bc1.numpy())}")
        print(f"r_bc1_xx:{np.mean(r_bc1_xx.numpy())}")
        print(f"r_data:{np.mean(r_data.numpy())}")
        print(f"r_physic:{np.mean(r_physic.numpy())}")
...
if __name__ == "__main__":
    ...
    a = 2.0  # Advection speed c
    l = 0.1  # Diffusion coefficient D
    X, T = np.meshgrid(x, t)
    ...
    #Exact = np.exp(-(10 * np.pi * 0.05)**2 * T) * np.sin(10 * np.pi * X)
